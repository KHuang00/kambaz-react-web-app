...
export default function WorkingWithArraysAsynchronously() {
    ...
    return (
        ...
        //                 <FaPencil onClick={() => editTodo(todo)} className="text-primary float-end me-2 me-3" />
                ...
                {todos.map((todo) => (
                    ...
                    <ListGroup.Item key={todo.id} className="d-flex align-items-center">
                        <input type="checkbox" className="form-check-input me-2"
                               ...
                               onChange={(e) => updateTodo({ ...todo, completed: e.target.checked })}/>
                        {!todo.editing ? (
                            <span className="flex-grow-1" style={{ textDecoration: todo.completed ? "line-through" : "none" }}>
                            {todo.title}
                        </span>
                        ) : (
                            // @ts-ignore
                            <FormControl className="flex-grow-1" defaultValue={todo.title}
                                         onKeyDown={(e) => {
                                             if (e.key === "Enter") {
                                                 updateTodo({ ...todo, editing: false });
                                             }
                        ...
                        <FaPencil onClick={() => editTodo(todo)} className="text-primary ms-auto me-2" />
                        <TiDelete onClick={() => deleteTodo(todo)} className="text-danger fs-3 me-2" id="wd-delete-todo" />
