...
export default function Courses() {
    ...
    useEffect(() => {
        if (!currentUser) {
            console.log(" Waiting for Redux to load `currentUser`...");
            return;
        }

        console.log("🔎 Checking enrollments for user:", currentUser._id);

        // Ensure enrollment user IDs match `currentUser._id`
        ...
        // Ensure course IDs match enrollments
        const filteredCourses = allCourses.filter(course =>
        ...
        // Set state only when courses are found
