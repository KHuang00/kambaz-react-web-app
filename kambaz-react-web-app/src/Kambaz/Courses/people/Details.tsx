...
import { IoCloseSharp } from "react-icons/io5";
import * as client from "../../Account/client";
import {removeUser, setAllUsers, setSelectedUser, updateUser} from "../../Account/reducer";
import {FaPencil} from "react-icons/fa6";
import {FormControl} from "react-bootstrap";
...
export default function PeopleDetails() {
    ...
    const user = useSelector((state: any) => state.account.selectedUser);
    ...
    const saveUser = async () => {
        ...
        // Refresh full user list in Redux
    ...
    return (
        <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
            ...
            {/*<button onClick={() => navigate(-1)} className="btn btn-secondary float-start me-2 wd-cancel">Cancel</button>*/}
            {/*<button onClick={() => deleteUser(uid)} className="btn btn-danger float-end wd-delete">Delete</button>*/}
            ...
            {editing && (
                <>
                    {/*@ts-ignore*/}
                    <FormControl
                        className="w-100 wd-edit-name mt-2 mb-2"
                        value={name}
                        onChange={(e) => setName(e.target.value)}
                        onKeyDown={(e) => e.key === "Enter" && saveUser()}
                    />
